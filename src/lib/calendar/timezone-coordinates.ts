/**
 * Representative coordinates for every IANA time zone, plus the legacy aliases that carry none
 * of their own.
 *
 * Generated from the tz database's public-domain zone.tab, zone1970.tab and backward files.
 * Each pair is the zone's "principal location" — the city the zone is named for, not a centroid
 * — which is the best available stand-in for somebody who has told us their zone and no more.
 *
 * Both tables are read because they answer different questions: zone1970.tab lists only zones
 * that have actually differed since 1970, so Europe/Amsterdam is absent from it and would
 * otherwise inherit Brussels. zone.tab still carries a row per name.
 *
 * Aliases matter because engines disagree on canonical names: a device in India reports
 * Asia/Calcutta through Intl on most of them, while the tz files call it Asia/Kolkata.
 *
 * Checked in rather than fetched at build time — the data changes rarely, and a zone's city
 * moving a few kilometres cannot shift a sunrise enough to matter here.
 */
...
/** Legacy zone name -> the canonical name carrying the coordinates. */
